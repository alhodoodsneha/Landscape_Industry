...
class EstimationTeamMember(models.Model):
    ...
    def _get_group_user_ids(self):
        group = self.env.ref('landscape_construction.group_estimation_manager')
        return group.user_ids.ids if group else []

    @api.model
    def _get_group_est_user_ids(self):
        group = self.env.ref('landscape_construction.group_estimation_user')
        return group.user_ids.ids if group else []
